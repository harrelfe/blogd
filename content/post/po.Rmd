...
Here is a graph that allows easy conversion from one metric to another.  The axes are, in order from top to bottom, the log odds ratio (treatment regression coefficient), the c-index (probability of treatment B having higher outcomes that treatment A), and odds ratio (anti-log of OR).
...
axis(1, at=lor, labels=as.character(lor),
     tcl=.5, mgp=c(-3,-1.7,0), line=-5)
axis(1, at=lor2, tcl=.25, col.ticks=gray(.7), labels=FALSE, line=-5)
title(xlab=expression(hat(beta)), line=-8)
```

As discussed in [Agresti and Tarantola](https://onlinelibrary.wiley.com/doi/10.1111/stan.12130), even with adjustment for covariates, if one compares a random patient on treatment B with a random patient on treatment A, with both patients having the same covariate values, the concordance probability is approximately $\frac{1}{1 + \exp{(-\beta/\sqrt{2})}}$ whereas our slightly better approximation replaces $\sqrt{2} =$ `r round(sqrt(2), 2)` with 1.52.
