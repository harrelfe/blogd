...
sapply(X, nrow)
# For each variable name count the number of datasets cantaining it
...
Here is a protypical `ggplot2` example illustrating many of the features I most often use.  Ignore the `ggplot2` `label` attribute if not using `plotly`.  Options are given to the `Hmisc` `label` function so that it will retrieve the variable `label` and `units` (if present) and format them for axis labels or tables.
...
for(i in 1 : nrow(R)) {  # took 4s
  ...
  cors    <- cor(X)
  maxr    <- max(cors[row(cors) < col(cors)]) # use upper triangle
  cors    <- cor(X, method='spearman')
  maxrho  <- max(cors[row(cors) < col(cors)])
  set(R, i, 'maxr',   maxr)    # set is in data.table & is very fast
  set(R, i, 'maxrho', maxrho)
}
```
...
g <- ggplot(u, aes(x=n, y=r, col=type)) + geom_jitter(height=0, width=1) +
      facet_wrap(~ p) +
      guides(color=guide_legend(title='')) +
      ylab('Mean Maximum Correlation Coefficient')
ggplotlyr(g)    # or plot::ggplotly(g) or just g for static graph
```
...
for(i in 1 : nsim) {   # took 1s
  for(n in ns) {
    for(p in ps) {
      X <- matrix(rnorm(n * p), ncol=p)
      for(type in c('r', 'rho')) {
        cors <- cor(X, method=switch(r = 'pearson', rho = 'spearman'))
        maxr <- max(cors[row(cors) < col(cors)])
