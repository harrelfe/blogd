...
<!-- Needed to make hovertext work with mermaid diagrams -->
<style>
  .mermaidTooltip {
      position: absolute;
      text-align: center;
...
s <- support[, lapply(.SD, is.na), .SDcols=vars]
# Hierarchical exclusions
# Instead of seqFreq(age, sex, dzgroup, ..., noneNA=TRUE) save typing
# by using do.call to build all those arguments
s[, exc := do.call('seqFreq', c(.SD, list(noneNA=TRUE)))]

# Create tooltip text from tabulation created by seqFreq
oc   <- attr(s[, exc], 'obs.per.numcond')
...
makemermaid(x,
  N0        = nrow(s),
  exclsep   = exclnodes(exc, from='Enr', seq=TRUE, remain=TRUE),
  excltab   = excltab
)
]
```
...
d[y > 2 & z > 3]      # rows satisfying condions
