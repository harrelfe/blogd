...
function](https://www.rdocumentation.org/packages/Hmisc/versions/4.7-0/topics/upData).  Here is an example:
...
The `Hmisc` `describe` function is my main tool for getting initial descriptive statistics and quality controlling the data in a univariate fashion.  Here is an example.  The `Info` index is a measure of the information content in a numeric variable relative to the information in a continuous numeric variable with no ties.  A very low value of `Info` will occur when a highly imbalanced variable is binary[`Info` comes from the [approximate formula](http://hbiostat.org/bib/r2.html) for the variance of a log odds ratio for a proportional odds model/Wilcoxon test, due to Whitehead.]{.aside}.

[`Gmd` stands for Gini's mean difference---the mean absolute difference over all possible pairs of different observations.  It is a very interpretable measure of dispersion that is more robust than the standard deviation.]{.aside}
```{r desc,results='asis'}
w <- describe(d)
html(w)
```
...
plot(s, which='categorical', vars=1 : 4)   # plot first 4 categorical variables
plot(s, which='continuous',  vars=1 : 4)   # plot first 4 continuous variables
```
:::
...
The [`data.table`](https://hbiostat.org/R/data.table) package provides a concise, consistent syntax for managing simple and complex data manipulation tasks, and it is extremely efficient for large datasets.  One of the best organized tutorials is [this](https://riptutorial.com/data-table), and a master cheatsheet for `data.table` is [here](https://rdrr.io/cran/data.table/man/data.table.html) from which the general syntax below is taken, where `DT` represents a data table.
...
Use a different function that uses `html` line breaks.  Tabulate all variables having between 3 and 10 distinct values.  Output a `markdown` format table.[`results='asis'` was added to the header of this code chunk.]{.aside}
```{r diffreq,results='asis'}
tab <- function(x) {
  # html to make frequencies be right justified
  hfill <- function(u) paste0("<span style='float: right; text-align: right;'>", u, "</span>")
  z <- table(x, useNA='ifany')
  paste(paste0(names(z), hfill(z)), collapse='<br>')
}
discr <- function(x) { i <- length(unique(x)); i > 2 & i < 11 }
w <- d[, lapply(.SD, tab), .SDcols=discr]
cat(paste(' ', names(w), collapse=' |'), '\n',
    paste(rep('--', length(w)), collapse='|'), '\n',
    paste(w, collapse='|'), '\n',
    ': {tbl-colwidths="[8,9,20,20]"}\n', sep='')
```
...
```{r two}
b <- data.table(id=1:4, age=c(21, 28, 32, 23))
...
b[L, on=.(id)]   # Keeps all ids in L
L[b, on=.(id)]   # Keeps all ids in b
L[b, on=.(id), nomatch=NULL]  # Keeps only ids in both b and L
```

You can also use the `merge` function for merging data tables.  This calls `merge.data.table`.  To merge any number of datasets at once and obtain a printed report of how the merge went, use the `Hmisc` `Merge` function.
