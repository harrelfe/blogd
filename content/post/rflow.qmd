...
Useful report building tools are gathered into the `rscripts` `reptools.r` script: `htmlList`, `kabl`, `maketabs`, `makecolmarg`, `makecnote`, `makecallout`, `makecodechunk`, `htmlView*`, `dataChk`, `missChk`, `scplot`.  These are all loaded when `reptools.r` is loaded.
...
|`varType`    |classify variables in a data table/frame as continuous, discrete, or non-numeric non-discrete|
...
types <- varType(support,
                 exclude=.q(slos, charges, totcst, totmcst, avtisst,
