...
* [`rscripts` Github repository](https://github.com/harrelfe/rscripts) with utility functions such as
   ...
   + `rsHelp` for viewing helps files for functions in `rscripts`
	 + `makemermaid` for making `mermaid` diagrams with R variable insertions
...
Useful report building tools are gathered into the `rscripts` `reptools.r` script: `htmlList`, `kabl`, `maketabs`, `makecolmarg`, `makecnote`, `makecallout`, `makecodechunk`, `htmlView*`, `dataChk`, `missChk`, `makemermaid`, `vClus`, `dataOverview`, `scplot`.  These are all loaded when `reptools.r` is loaded.
...
The likelihood ratio $\chi^2$ test in the last tab shows strong evidence that the number of missing variables per patient is associated with the values of some of the non-missing variables.  From the dot chart we see that the strongest predictors of missing baseline variables are time to death/censoring and disease group.  This may be due to patients on ventilators not being able to provide as much baseline information such as activities of daily living (`adlp`), and being on a ventilator is a strong prognostic sign.   There is a possible sex effect worth investigating.
...
Now show the flow of qualifications and exclusions in a consort diagram.  `consort_plot` wants multiple reasons for exclusions to be prioritized in a hierarchy, and `NA` to be used to denote "no exclusion".  Define a function `dexclude` the creates a `factor` variable whose first level is the most common exclusion, second level is the most common exclusion after excluding subjects based on the first exclusion, and so on.
...
```{r consort}
dexclude <- function(..., labels, noneNA=FALSE) {
  x <- cbind(...)
  k <- ncol(x)
  if(k != length(labels)) stop('invalid labels')
  j <- integer(k)
  w <- rep(0, nrow(x))
  for(i in 1 : k) {
    freqs <- apply(x, 2, sum, na.rm=TRUE)
...
j <- j[j != 0]
if(noneNA) factor(w, j, labels[j]) else
           factor(w, c(0, j), c('none', labels[j]))  
}
r[, exc := dexclude(pain == 0, hxmed == 1, age < 40,
                    labels=c('pain-free', 'Hx med', 'age < 40'),
                    noneNA=TRUE)]
...
The [`mermaid`](https://mermaid-js.github.io/mermaid) natural diagramming language can be used to make `consort`-like flowcharts among many other types of diagrams.  `Quarto` has [built-in support for `mermaid`](https://quarto.org/docs/authoring/diagrams.html), and a `reptools` function `makemermaid` uses the capabilities of the `knit_expand` function in the `knitr` package to allow variable values to easily be inserted into `mermaid` specifications using the `{{variablename}}` notation which is also used in one example to insert several program-generated lines of `mermaid` commands.  `results='asis'` needs to be in the chunk header to use `makemermaid`, and any node labels that contain parentheses must be enclosed in quotes.  In the first example, exclusions are in one large node and a special output class (used to limited effect) is used for this node.  In the second example, each exclusion is a subnode to the overall count of exclusions.
...
x <- 'flowchart TD
  ...
  Q   --> C["Consented (n={{Nc}})"]
  C   --> R["Randomized (n={{Nr}})"]
  R   --> TxA["Treatment A (n={{Ntxa}})"]
  R   --> TxB["Treatment B (n={{Ntxb}})"]
...
makemermaid(x,
            ...
            Nc   = count(consent),
            Nr   = count(randomized),
            Ntxa = count(tx == 'Treatment A'),
            Ntxb = count(tx == 'Treatment B'),
            Ntxaf= count(tx == 'Treatment A' & fin),
            Ntxbf= count(tx == 'Treatment B' & fin),
            Ntxao= count(tx == 'Treatment A' & ! is.na(y)),
            Ntxbo= count(tx == 'Treatment B' & ! is.na(y))
            )
]
```

```{r results='asis'}
exclnodes <- function(x, seq=FALSE) {
  # Create complete node specifications for individual exclusions, each
  # linking to overall exclusion count assumed to be in node E
  ...
  tab <- table(x)
  i <- 1 : length(tab)
  w <- paste0('["', names(tab), ' (n=', tab, ')"]')
  u <- if(seq) ifelse(i == 1, paste0('E --> E1', w),
                              paste0('E', i-1, '--> E', i, w))
    else paste0('E --> E', i, w)
  paste(u, collapse='\n')
}
