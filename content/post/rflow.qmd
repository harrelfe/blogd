...
Useful report building tools are gathered into the `rscripts` `reptools.r` script: `htmlList`, `kabl`, `maketabs`, `makecolmarg`, `makecnote`, `htmlView*`, `dataChk`, `scplot`.  These are all loaded when `reptools.r` is loaded.
...
To make creation of tabs in `Quarto` easier, the `maketabs` function loops through the elements of a sequence of formulas, runs the commands (separately by `+` if more than one) just after the ~, outputs each element into a separate `Quarto` tab.  The label for the tab is taken as the left hand side of the formula, which must be enclosed in backticks and not quotes if it is a multi-word string.  If `+ raw` in the formula's right side, the chunk assumes the command's output is raw text.  A `wide` argument is used to expand the width of the output outside the usual margins.  An `initblank` argument creates a first tab that is empty.  This allows one to show nothing until one of the other tabs is clicked. Alternately you can specify as the first formula `` `` ~ `` ``. [`maketabs` was loaded by the earlier `getRs('reptools.r', put='source')` command and its source code may be viewed [here](https://github.com/harrelfe/rscripts/blob/master/reptools.r).]{.aside}  `maketabs` can also be called with a named list instead of a series of formulas.  The list names become tab labels.
...
Write a function to compute several model summaries and put them in tabs.  `raw` in a formula makes the generated R chunk include output in raw format.
