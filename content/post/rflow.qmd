...
Now consider moving estimates, least squares (OLS), ordinal regression (ORM), and quantile regression (QR) estimates, and nonparametric `loess` estimates.  Estimates are made from the 10th smallest to the 10th largest age to avoid extrapolation.  The mean and all three quartiles are estimated.  For each target age value the window for estimating creatinine is the target ± 5y.  Age is incremented by 0.5y for the next window.  All this is obtained by the `movStats` function [on Github](https://github.com/harrelfe/rscripts/blob/master/movStats.r), which makes heavy use of `data.table` and the `rms` packages.  For ordinal regression estimates of the mean and quantiles the log-log link is used.  Moving estimates are shown with and without [`supsmu`](https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/supsmu)-smoothing them.

```{r moving}
#| fig.height: 5
#| fig.width: 7
getRs('movStats.r', put='source')
u <- movStats(crea ~ age, eps=5, xinc=0.5,
...
Let's describe how white blood count relates to the probability of hospital death, using a binary logistic regression model and moving proportions with a ± 0.4 tolerance on `r cwbc`, and getting estimates every 0.025 `r cwbc` units.  The cube root transformation is used because of the extreme skewness of WBC, resulting in data being too sparse for high WBC for a fixed-window moving proportion to work well.  Use 6 knots at default locations on `r cwbc`. 

```{r wbc}
u <- movStats(hospdead ~ wblc, eps=0.4, xinc=0.025, k=6,
...
The flexibility of the moving statistic method is demonstrated by estimating how age relates to probabilities of death within 1y and within 2y using Kaplan-Meier estimates in overlapping moving windows.  Assumptions other than smoothness (e.g., proportional hazards) are avoided in this approach.  Here is an example that also uses an flexible parametric method, [hazard regression](https://www.tandfonline.com/doi/abs/10.1080/01621459.1995.10476491), implemented in the R [`polspline`](https://cran.r-project.org/web/packages/polspline/index.html) package, that adaptively finds knots (points of slope change) in the covariate and in time, and products of piecewise linear terms so as to allow for non-proportional hazards.  We use far less penalization that is the default for the `hare` function for demonstration purposes.  For this dataset the default settings of `penalty` and `maxdim` result in straight lines.

```{r km}
u <- movStats(Surv(d.time / 365.25, death) ~ age,
              eps=2, xinc=0.5, times=1:2,
...
```{r km2}
u <- movStats(Surv(d.time / 365.25, death) ~ age + dzclass,
              eps=3, xinc=0.5, times=1:2,
              msmooth='both', hare=TRUE,
              melt=TRUE, data=support)
...
Mimic these results using flexible regression with interaction.  Start by estimating the mean.  Add spike histograms to estimated trend curves.  Spike heights are proportional to the sample size in age/race-ethnicity groups after binning age into 100 bins.
...
anova(f)
ggplot(Predict(f, age, re), rdata=nhgh,
       histSpike.opts=
         list(frac=function(f) 0.1 * f / max(f),
              side=1, nint=100))
# For some reason can't add theme or guides to ggplot.Predict output
```
