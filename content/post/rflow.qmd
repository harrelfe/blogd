...
Trends estimated this way are more accurate than those estimated from simply stratifying on age, and there were minimal assumptions.  In general the fewest assumptions are made when using known unbiased estimates (in this case the sample mean and sample median) are computed on overlapping moving windows (here, of age) that advance by a small amount (here 0.5 years) each time.  Estimates are made from the 10th smallest to the 10th largest age to avoid extrapolation.  For each target age value the window for estimating creatinine is the target ± 5y.  All this plus the two regression fits and `loess` nonparametric smoothers estimating the mean as a function of age are obtained by the `movStats` function [on Github](https://github.com/harrelfe/rscripts/blob/master/movStats.r), which makes heavy use of `data.table` and the `rms` package.
