...
A better approach is to let the `kables` function put together a series of separate `markdown` tables of different sizes.

```{r tabalt}
tab <- function(x) {
  z <- table(x, useNA='ifany')
  i <- length(Z)
  w <- cbind(names(z), z); rownames(w) <- NULL
  colnames(w) <- c(vnames[i+1], 'Freq')
