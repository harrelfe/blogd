...
Useful report building tools are gathered into the `rscripts` `reptools.r` script: `htmlList`, `kabl`, `maketabs`, `makecolmarg`, `htmlView*`, `dataChk`, `scplot`.  These are all loaded when `reptools.r` is loaded.
