...
```{r summaryM2}
# Plot first 4 categorical and continuous variables
plot(s, which='categorical', vars=1 : 4, height=600, width=1000)
```
...
d[, x := factor(x, .q(a1,a2,b1,b2,b3), .q(a,a,b,b,b))]
...
d[, x := score.binary(x1 == 'symptomatic', x2 %in% c('stroke', 'MI'), death)]
# Same but code with numeric points  (death is a TRUE/FALSE variable)
d[, x := score.binary(x1 == 'symptomatic', x2 %in% c('stroke', 'MI'), death,
                      points=c(1,2,10))]
...
# or do a merge for table lookup (see below)
