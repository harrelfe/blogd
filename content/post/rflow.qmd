...
* Nonparametric smoothers are excellent tools for relating individual continuous variables to an outcome.
...
Analyses should shed light on the unknown and not dwell on the known.  In a randomized trial, the distributions of baseline variables are expected to be the same across treatments, and will be the same once $N$ is large.  What is unknown is how the subject characteristics (and treatment) relate to the outcomes under study.  While displaying this trend with a nonparametric smoother, one can simultaneously display the marginal distribution of the characteristic using an extended box plot, spike histogram, or rug plot.  Here is an example using the `Hmisc` `summaryRc` function.  Extended box plots show the mean, median, and quantiles that cover 0.25, 0.5, 0.75, and 0.9 of the distribution.[For more examples see [this](http://hbiostat.org/R/Hmisc/summaryFuns.pdf)]{.aside}
...
Here is an example in which the distribution of continuous variables is shown, stratified by disease group.
...
To understand the relationship between two continuous variables we may desire to estimate the mean (using ordinary least squares) and median (using quantile regression) as a smooth function of a predictor (using a restricted cubic spline fit).

```{r meanq}
require(rms)
ddist <- datadist(support); options(datadist='ddist')
f  <- ols(crea ~ rcs(age, 5), data=support)
p1 <- Predict(f)
g  <- Rq(crea ~ rcs(age, 5), tau=0.5, data=support)
p2 <- Predict(g)
p  <- rbind('OLS Mean'=p1, 'QR Median'=p2)
ggplot(p, groups='.set.')
```

Trends estimated from this plot are more accurate than those estimated from simply stratifying on age, and there were minimal assumptions.  In general the fewest assumptions are made when using known unbiased estimators (in this case the sample mean and sample median) computed on overlapping moving windows (here, of age) that advance by a small amount (here 0.5 years) each time.  Estimates are made from the 10th smallest to the 10th largest age to avoid extrapolation.  For each target age value the window for estimating creatinine is the target age ± 5y.  A `data.table` non-equi join handles the fuzzy matching into target age intervals.

```{r moving}
s      <- as.data.table(support)[, .(age, crea)]
arange <- s[, round(c(sort(age)[10], sort(age, decreasing=TRUE)[10]))]
aseq   <- seq(arange[1], arange[2], by=0.5)
# Same as
# arange <- round(ddist$limits[c('Low:prediction', 'High:prediction'), 'age'])
a <- data.table(tage=aseq, key='tage')    # target ages for estimation
a[, .q(lo, hi) := .(tage - 5, tage + 5)]  # define all windows
m <- a[s, on=.(lo <= age, hi >= age)]     # non-equi join
setkey(m, tage)
g <- function(x) list('Sample Mean'   = mean(x, na.rm=TRUE),
                      'Sample Median' = median(x, na.rm=TRUE))
u <- m[, g(crea), by=tage]
w <- melt(u, id.vars='tage',
          measure.variables=c('Sample Mean', 'Sample Median'),
          variable.name='Statistic', value.name='Creatinine')
# Also compute loess estimates
np <- loess(crea ~ age, data=s)
pc <- predict(np, data.frame(age=aseq))
w  <- rbind(w, data.table(tage=aseq, Creatinine=pc, Statistic='loess'))
# Add estimates from OLS and quantile regression
setDT(p)  # make it a data.table
w  <- rbind(w, p[, .(tage=age, Creatinine=yhat, Statistic=.set.)])
ggplot(w, aes(x=tage, y=Creatinine, color=Statistic)) + geom_line() +
  xlab('Age')
```
