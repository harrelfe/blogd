...
L[, consecutive := z > 0.5 & shift(z) > 0.5, by=id][, 
...
```{r ggplot2ex}
# Naming the label txt will make ggplotlyr work
d[, txt := paste0('dose:', dose)]
g <-
  ggplot(d, aes(x=age, y=bhr, color=gender, label=txt)) +
...
ggplotlyr(g)   # dose is in hover text for each point
```
