...
Useful report building tools are gathered into the `rscripts` `reptools.r` script: `htmlList`, `kabl`, `maketabs`, `makecolmarg`, `makecnote`, `makecallout`, `makecodechunk`, `htmlView*`, `dataChk`, `scplot`.  These are all loaded when `reptools.r` is loaded.
...
The `Hmisc` package [`curveRep`](https://www.rdocumentation.org/packages/Hmisc/versions/4.7-0/topics/curveRep) function facilitates the latter.  It is important to consider per-subject sample sizes and measurement time gaps as these will not only limit how we look at the data but may be informative, e.g., subjects who are failing may start to get more frequent measurements.
...
::: {.callout-note collapse="true"}
## Alternative to Manually Selecting Variables

Sometimes it is worth having a function do the work of determining which variables to treat as continuous and which ones to treat as categorical.  Here is one such function.

```{r vartype}
vartype <- function(data, include=NULL, exclude=NULL, ndistinct=10) {
  ...
  g <- function(x) 
    if(is.character(x) || is.factor(x) ||
       (is.numeric(x) && length(unique(x)) <= ndistinct))
      'discrete' else 'continuous'
  s <- sapply(data[v], g)
  split(names(s), s)
}
types <- vartype(support, exclude=.q(d.time, death, hospdead, sfdm2))
print(types, quote=FALSE)
```

:::

In addition to showing all the estimated relationships with the outcome, put covariate distributions in the margin.[Note the bimodality of some of the measurements, and true zero blood pressures for patients having cardiac arrest.]{.aside}

```{r movall,results='asis'}
U <- NULL
V <- .q(age, meanbp, wblc, hrt, temp, alb, crea, sod)
for(v in V) {
  support$.x. <- support[[v]]
  x <- support[[v]]
  u <- movStats(Surv(d.time / 365.25, death) ~ x, times=1:2,
              ...
              melt=TRUE, data=support)
  u$vname <- label(x, default=v)
  U <- rbind(U, u)
}
ggplot(U, aes(x, y=incidence, col=Statistic, linetype=Type)) + geom_line() +
  facet_wrap(~ vname, scales='free_x') +
  ylab(label(u$incidence)) + xlab('') +
  guides(color=guide_legend(title='')) +
  theme(legend.position='bottom')
# Use plot(describe()) to get spike histograms of these vars.
require(plotly)   # for %>%
...
```{r allcat}
U <- NULL
V <- .q(sex, dzgroup, adls, race)
for(v in V) {
  support$.x. <- support[[v]]
  u <- movStats(Surv(d.time / 365.25, death) ~ .x., times=1:2,
                discrete=TRUE, melt=TRUE, data=support)
  lab <- label(support$.x., default=v)
  u[, vname := lab]
  U <- rbind(U, u)
}
ggplot(U, aes(x=incidence, y=.x., col=Statistic)) + geom_point() +
...
w <- split(U, U$vname)   # split into one data table per x variable
g <- lapply(w, gg)       # put 'em all together to a list of ggplot objects
maketabs(g)
```
