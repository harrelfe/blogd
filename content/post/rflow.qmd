...
* If there is to be a pivotal analysis there should be a statistical analysis plan (SAP) for this analysis that does not allow for many "statistician degrees of freedom."  The plan should be completed before doing any analysis that might inform the plan in a way that would bias the results (e.g., bias the estimate of treatment effect or bias standard errors of effects in a model).
...
Analyses should shed light on the unknown and not dwell on the known.  In a randomized trial, the distributions of baseline variables are expected to be the same across treatments, and will be the same once $N$ is large.  What is unknown is how the subject characteristics (and treatment) relate to the outcomes under study.  While displaying this one can simultaneously display the marginal distribution of the characteristic using an extended box plot, spike histogram, or rug plot.  Here is an example using the `Hmisc` `summaryRc` function.  Extended box plots show the mean, median, and quantiles that cover 0.25, 0.5, 0.75, and 0.9 of the distribution.[For more examples see [this](http://hbiostat.org/R/Hmisc/summaryFuns.pdf)]{.aside}
...
Here is an example in which the distribution of continuous distributions is shown, stratified by disease group.
...
p2 <- Predict(g)
p  <- rbind(Mean=p1, Median=p2)
ggplot(p, groups='.set.')
```

Trends estimated from this plot are more accurate than those estimated from simply stratifying on age.

## Formatting

I take advantage of special formatting for model fit objects from the `rms` package by using `html` or `latex` methods and putting `results='asis'` in the chunk header to preserve the special formatting.  Here is an example.
