...
* [`rscripts` Github repository](https://github.com/harrelfe/rscripts) with utility functions that are all loaded when `reptools.r` is loaded
   ...
   + `dataChk` for data checking
	 ...
	 + `ebplayers` for adding extended box plot layers to `ggplot2` plots, especially when run on the output of `meltData`
...
Here is an example using the `reptools` `meltData` and `ebplayers` functions.  Extended box plots show the mean (blue dot), median, and quantiles that cover 0.25, 0.5, 0.75, and 0.9 of the distribution.  In addition to standard extended box plot quantiles, we show the 0.01 and 0.99 quantiles as dots.[For more examples see [this](http://hbiostat.org/R/Hmisc/summaryFuns.pdf)]{.aside}


```{r table1}
#| label: fig-table1
#| fig-cap: "`ggplot2` nonparametric smooth estimated relationships between continuous baseline variables and the probability that a patient in the ICU will die in the hospital.  Extended box plots are added to the panels, with points added for 0.01 and 0.99 quantiles."
#| fig-height: 4.5
#| fig-width: 6.25
addCap()
