...
In the second `mermaid` example, each exclusion is a subnode to the overall count of exclusions, and a node is added to inform us about multiple exclusions per subject.  Let's also remove the need for more error-prone manual coding of parallel treatment groups.
...
makemermaid(x,
  ...
  finished = parNodes(count(fin, by=tx), 'F',  fromparallel='Tx',
                      label='Finished'),
  outcome  = parNodes(count(! is.na(y), by=tx),
                      'O',fromparallel='F', label='Outcome assessed')
)
]
```
