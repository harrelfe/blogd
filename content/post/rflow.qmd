...
ggp <- if(outfmt != 'html')
  function(ggobject, ...) ggobject else ggplotlyr
...
```{r eval=FALSE}
mNote <- if(outfmt == 'html') '.column-margin'
  else
    '.callout-note appearance="minimal"'
```

Then use <tt>r mNote</tt> enclosed in backticks in place of the `.column-margin` callout for generality.
...
m               <- subset(m, name %in% n)
labs            <- m$label
un              <- m$units
names(labs)     <- names(un) <- m$name
...
The built-in function in R for reading `.csv` files is [`read.csv`](https://www.rdocumentation.org/packages/utils/versions/3.6.2/topics/read.table).  The `Hmisc` package has a function [`csv.get`](https://www.rdocumentation.org/packages/Hmisc/versions/4.7-0/topics/csv.get) which calls `read.csv` but offers some enhancements in variable naming, date handling, and reading variable labels from a specified row number.  The [`fread`](https://www.rdocumentation.org/packages/data.table/versions/1.14.2/topics/fread) function in the `data.table` package is blazing fast for reading large files and offers a number of options.
...
d[, x := fcase(death,                  'death',        # takes precedence
               x2 %in% .q(stroke, MI), 'stroke/MI',    # takes next precendence
...
The `cube` function in the `data.table` package will compute all possible marginal statistics.  When there is only one `by` variable, the overall statistical is computed in addition to compute stratified values.  When a dimension is being marginalized over, the value of the `by` variable for that dimension will be `NA`.
...
Now consider an xample where there is an "events" dataset `e` with 0 or more rows per
